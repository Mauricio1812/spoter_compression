# python -m train_mediapipe --epochs 15 --validation_set split-from-train --experiment_name LSA64_6L --num_layers 6 --num_classes 64 --dataset LSA64
# python -m train_mediapipe --epochs 15 --validation_set split-from-train --experiment_name DGI406_6L --num_layers 6 --num_classes 449 --dataset DGI305
import os
import argparse
import random
import logging
...
def get_default_args():
    ...
    parser.add_argument("--experiment_name", type=str, default="autsl_255_spoter_exp",
...
def train(args):
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('out-checkpoints/' + args.experiment_name + "/"+ args.experiment_name + "_" + str(args.experimental_train_split).replace(".", "") + ".log")
        ]
    )
    ...
    # MARK: TRAINING
    # Construct the model
    slrt_model = SPOTER(num_classes=args.num_classes, encoder_layers=args.num_layers, decoder_layers=args.num_layers, hidden_dim=args.hidden_dim)
    ...
    if eval_loader:
        for i in range(checkpoint_index):
            for checkpoint_id in ["t", "v"]:
                ...
                _, _, eval_acc = evaluate(tested_model, eval_loader, device, print_stats=True, num_classes=args.num_classes)

                if eval_acc > top_result:
                    top_result = eval_acc
                    top_result_name = args.experiment_name + "/checkpoints" + checkpoint_id + "_" + str(i)
    ...
    if args.plot_stats:
        ...
        fig.savefig("out-img/" + args.experiment_name + "/" + args.experiment_name + "_loss.png")
